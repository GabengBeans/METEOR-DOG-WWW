<template>
    <div>
        ...
        <quality-page :storeStatus="status" :currentPage="$store.state.app.new_model_ip_page_info.currentPage" :totalPage="$store.state.app.new_model_ip_page_info.totalPage"></quality-page>
...
<script>
import qualityTable from "./new_model_ip_table.vue";
        ...
        .then(resp => {
          if (resp.data.success) {
            this.$store.commit("GET_NEW_MODEL_IP_LIST", {
                ...
                pageNo: this.$store.state.app.new_model_ip_public_page
            });
            this.$Message.destroy();
