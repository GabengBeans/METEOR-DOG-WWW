...
<script>
    const detailButton = (vm, h, currentRowData, name) => {
    ...
    const editButton = (vm, h, currentRowData, name) => {
        return h('Button', {
            props: {
                type: 'primary',
            },
    ...
    const deleteButton = (vm, h, index, currentRowData) => {
        return h('Poptip', {
            props: {
                confirm: true,
                title: '您确定要删除这条数据吗?',
    ...
    const showButton = (vm, h, index,currentRowData, items) =>{
        let buttonItems = [];
        items.forEach(x=> {

            switch (x.options){
                case 'detail':
                    buttonItems.push(detailButton(vm, h, currentRowData, x.name));
                    break;
                case 'edit':
                    buttonItems.push(editButton(vm, h, currentRowData, x.name));
                    break;
                case 'delete':
                    buttonItems.push(deleteButton(vm, h, index, currentRowData));
                    break;
            }
        });
                ...
                this.columnsList.forEach(item => {
                    ...
                    if (item.handle) {
                        item.render = (h, param) => {
                            ...
                            let buttonList = showButton(this, h, param.index, currentRowData ,item.handle);
                            // console.log(item);
